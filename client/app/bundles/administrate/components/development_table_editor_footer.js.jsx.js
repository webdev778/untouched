export let DevelopmentTableEditorFooter = React.createClass({
  ...
  renderDropdownOptions(column) {
    let prop = this.props.properties[column.property];
    return _.map(prop.options, option => <option key={option.value} value={option.value}>{option.name}</option>
    );
  },
